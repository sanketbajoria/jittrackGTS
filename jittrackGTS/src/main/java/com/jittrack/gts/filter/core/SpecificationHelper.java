...
import static com.jittrack.gts.filter.core.SpecificationBuilder.and;
...
public final class SpecificationHelper {
    ...
    public static <E> AbstractSpecification<E> makeFilterPath(final String field ) {
    	String[] creteria=field.split("\\;");
    	List<AbstractSpecification<E>> filterParams = new ArrayList<AbstractSpecification<E>>();
    	for(String singleCreteria: creteria){
    		String[] operand=singleCreteria.split("\\,");
    		AbstractSpecification<E> spec;
    		if(operand.length==3)
    		{
    			 spec = filter(operand[0], operand[1],FilterOperator.EQ);

    		}
    		else
    		{
   			 spec = filter(operand[0], operand[1]);

    		}
    		filterParams.add(spec);
    	}
    	
    	
        return and(filterParams);
    }
